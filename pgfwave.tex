...
\pgfwavedef{LD}{-- ++(\slope,\height)     -- ++(\width-\slope,0)
               ++(-\width,-\height) ++(\zslope,+\height/2.)
            -- ++(\zslope,-\height/2.) -- ++(\width-\slope,0) }
\pgfwavelet{lD}{LD}
\pgfwavedef{Ld}{-- ++(\slope,\height)     -- ++(\width/2.-\slope,0)
               ++(-\width/2.,-\height) ++(\zslope,+\height/2.)
            -- ++(\zslope,-\height/2.) -- ++(\width/2.-\slope,0) }
\pgfwavelet{ld}{Ld}
...
\pgfwavedef{Dh}{   ++(0,+\height)
            ...
            -- ++(\slope,+\height) -- ++(\width/2.-\slope,0) }
\pgfwavelet{dh}{Dh}
\pgfwavedef{DD}{-- ++(+\width,0) ++(-\width*1.1,+\height) %++(-\width/2.,+\height/2.) node [scale=0.75] {A} ++(-\width/2.+1,+\height/2.)
            -- ++(+\width*1.1,0) ++(0,-\height) }
\pgfwavelet{dD}{DD}
\pgfwavedef{Dd}{-- ++(+\width/2.,0) ++(-\width*0.6,+\height)
            -- ++(+\width*0.6,0) ++(0,-\height) }
\pgfwavelet{dd}{Dd}
